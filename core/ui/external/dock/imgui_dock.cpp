...
struct DockContext {
      ...
      switch (dock_slot) {
         ...
         case ImGuiDockSlot_Top:
            return ImRect(rect.Min,
                          ImVec2(rect.Max.x, rect.Min.y + half_size.y));
         ...
         case ImGuiDockSlot_Left:
            return ImRect(rect.Min,
                          ImVec2(rect.Min.x + half_size.x, rect.Max.y));
      }
   }
      ...
      if (is_float) {
         ...
         bool ret = Begin(
             ...
             ImGuiWindowFlags_NoCollapse /*| ImGuiWindowFlags_ShowBorders*/ |
                 extra_flags);  // ImGuiWindowFlags_ShowBorders not used in new
                                // version of ImGui
   ...
   void debugWindow() {
      ...
      if (Begin("Dock Debug Info")) {
         for (int i = 0; i < m_docks.size(); ++i) {
            if (TreeNode(INT2VOIDP(i), "Dock %d (%p)", i, m_docks[i])) {
               Dock& dock = *m_docks[i];
...
int getDockIndex(const DockContext& context, DockContext::Dock* dock) {
   ...
   for (int i = 0; i < context.m_docks.size(); ++i) {
      if (dock == context.m_docks[i]) return i;
   }
   ...
   // specific size of docks
      ...
      for (int i = 0; i < dockSize; i++) {
         DockContext::Dock* new_dock =
             (DockContext::Dock*)MemAlloc(sizeof(DockContext::Dock));
         context.m_docks.push_back(IM_PLACEMENT_NEW(new_dock)
                                       DockContext::Dock());
      }
...
static void writeAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler,
   ...
   // Write a buffer
   buf->reserve(buf->size() + totalDockNum * sizeof(DockContext::Dock) +
                32 * (totalDockNum + (int)g_docklist.size() * 2));
...
void ImGui::ShutdownDock() {
   for (auto& iter : g_docklist) {
      ...
      for (int k = 0, dock_count = (int)context.m_docks.size(); k < dock_count;
           k++) {
