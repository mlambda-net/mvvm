...
static bool ImGui_ImplSDL2_Init(SDL_Window* window, SDL_Renderer* renderer) {
   ...
   (void)window;
      ...
      // SDL doesn't emit the event. Without this, when clicking to gain focus,
      // our widgets wouldn't activate even though they showed as hovered. (This
      // is unfortunately a global SDL setting, so enabling it might have a
      // side-effect on your application. It is unlikely to make a difference,
      // but if your app absolutely needs to ignore the initial on-focus click:
...
#define MAP_BUTTON(KEY_NO, BUTTON_NO)                                      \
   {                                                                       \
      io.AddKeyEvent(KEY_NO, SDL_GameControllerGetButton(game_controller,  \
                                                         BUTTON_NO) != 0); \
   }
